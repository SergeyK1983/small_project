...
def test_signup_user_create(mock_user, mock_is_user, client):
...
@patch("src.auth.services.register_service.UserRegisteredRepo.is_unique_user")
def test_signup_user_raise_409(mock_is_user, client):
...
def test_signup_user_invalid_form(client, form_data, field, expected):
...
def test_signup_user_invalid_data(client, data, expected):
